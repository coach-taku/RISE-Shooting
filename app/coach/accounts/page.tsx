...
// コーチが選手アカウントの作成・名前変更・パスワード変更・削除を行う画面。
//
...
export default function AccountsPage() {
  ...
  // コーチ名編集モーダル
  const [editCoachTarget, setEditCoachTarget] = useState<Profile | null>(null)
  const [editCoachUsername, setEditCoachUsername] = useState('')
  const [editingCoach, setEditingCoach] = useState(false)
  const [editCoachMsg, setEditCoachMsg] = useState('')
  ...
  // 削除確認ダイアログを開く
  const openDeleteModal = (player: Profile) => {
  ...
  const handleDelete = async () => {
    ...
    if (data.success) {
      ...
      setDeleteMsg(`❌ エラー: ${data.error}`)
      setDeleting(false)
    }
  }

  // コーチ名編集モーダルを開く
  const openEditCoachModal = (coach: Profile) => {
    setEditCoachTarget(coach)
  ...
  const handleEditCoach = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!editCoachTarget) return

    // 名前が変わっていなければ何もしない
    if (editCoachUsername.trim() === editCoachTarget.username) {
      setEditCoachMsg('⚠️ 変更内容がありません。')
      return
    }
    ...
    const res = await fetch('/api/admin/update-coach', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        targetUserId: editCoachTarget.id,
        newUsername: editCoachUsername.trim(),
      }),
    })
    const data = await res.json()

    if (data.success) {
      setEditCoachMsg('✅ 更新しました！')
      fetchData()
      setTimeout(() => {
      ...
      setEditCoachMsg(`❌ エラー: ${data.error}`)
    }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <p className="text-sm text-black/70 mb-6">選手の登録・名前変更・パスワード管理・削除、コーチ名の変更</p>
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="flex gap-2 ml-3 flex-shrink-0">
                      ...
                      <button
                        ...
                        className="text-xs px-3 py-1.5 rounded-lg font-medium transition-colors whitespace-nowrap"
                        style={{ backgroundColor: 'rgba(220,50,50,0.18)', color: '#7f1d1d' }}
                      >
                        🗑 削除
                      </button>
                    </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-base font-bold text-black">コーチ一覧</h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl p-6 text-center" style={{ backgroundColor: 'rgba(255,255,255,0.1)' }}>
            <p className="text-black/60">コーチアカウントがありません。</p>
          </div>
        ) : (
          <div className="rounded-2xl overflow-hidden shadow" style={{ backgroundColor: 'rgba(255,255,255,0.12)' }}>
            <div className="divide-y divide-white/10">
              {coaches.map((coach) => (
                <div key={coach.id} className="px-4 py-3">
                  <div className="flex items-center">
                    <div className="flex-1 min-w-0">
                      {/* コーチ名 */}
                      <p className="text-black font-bold truncate">{coach.username}</p>
                      <p className="text-xs text-black/40 mt-0.5">登録日: {coach.created_at.split('T')[0]}</p>
                    </div>
                    {/* 編集ボタン（コーチは削除不可） */}
                    <button
      ...
      {showDeleteModal && deleteTarget && (
        ...
        >
          <div
            ...
            style={{ backgroundColor: '#2a2a2a' }}
          >
            <h3 className="text-white font-bold text-lg mb-1">🗑 選手を削除しますか？</h3>
            <p className="text-white/70 text-sm mb-2">
              <span className="text-white font-bold">{deleteTarget.username}</span> さんのアカウントを削除します。
            </p>
            <p className="text-red-400 text-xs mb-5">
              ⚠️ この操作は取り消せません。過去のシューティング記録もすべて削除されます。
            </p>

            {deleteMsg && (
              <p className="text-sm text-white font-medium mb-3">{deleteMsg}</p>
            )}
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2 rounded-xl text-sm text-white/70 border border-white/20 disabled:opacity-40"
              >
                キャンセル
              </button>
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2 rounded-xl font-bold text-white text-sm disabled:opacity-60"
      ...
      {/* ===== コーチ名編集モーダル ===== */}
      {showEditCoachModal && editCoachTarget && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center px-4"
          style={{ backgroundColor: 'rgba(0,0,0,0.7)' }}
        >
          <div
            ...
            style={{ backgroundColor: '#2a2a2a' }}
          >
            <h3 className="text-white font-bold text-lg mb-1">✏️ コーチ名の変更</h3>
            <p className="text-white/60 text-sm mb-4">
              {editCoachTarget.username} さんの名前を変更します
            </p>
            <form onSubmit={handleEditCoach} className="space-y-4">
              ...
              {editCoachMsg && (
                <p className="text-sm text-white font-medium">{editCoachMsg}</p>
              )}
